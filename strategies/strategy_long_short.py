...
class LongShort(bt.Strategy):
    ...
    def all_indicators_ready(self):
        return len(self) >= max(self.atr.params.period,  self.ema_bollinger_bands.params.period, self.bollinger_bands.params.period, self.rsi.params.period, self.willr.params.period, self.rocr.params.period, self.sqrt_weighted_mean_volume_traded.params.period)


    '''
    def prenext(self):
        if self.all_indicators_ready():
            self.log('All indicators have enough data to produce valid outputs.')
        else:
            self.log('Waiting for all indicators to be ready.')
    '''
    ...
    def next(self):
        ...
        if not self.order_refs:
            ...
            if self.position.size == 0 :
                ...
                # NEGATIVE SIGNAL   (could be elif ?)
                if short_entry_signals >= 1:
    ...
    def close_position(self, reason):
        ...
        Parameters :
            reason : The reason why we need to close the position.
        """
        if self.position.size >= 0:
    ...
    def update_brackets_for_new_size(self, new_size):
        ...
        Parameters :
            new_size : The new size of the position after scaling due to position management.       
        """
        if not self.position:
            self.log('No active position to update brackets for.')
            return
